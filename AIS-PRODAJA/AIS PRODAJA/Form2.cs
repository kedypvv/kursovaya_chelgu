    ...
    public partial class Form2 : Form
    {
        public void GetListClients(ListBox listbox)
        {
            ...
            while (reader.Read())
            {
                ...
                string phone_client = reader[2].ToString();
                string email_client = reader[3].ToString();

                listbox.Items.Add($"{id_client}) ФИО: {fio_client} | Номер телефона : {phone_client} | EMAIL : {email_client}");
            }
            reader.Close();
            conn.Close();
        }
