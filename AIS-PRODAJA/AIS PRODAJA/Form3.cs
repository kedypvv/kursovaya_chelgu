...
namespace AIS_PRODAJA
{
    public partial class Form3 : Form
    {
        public void GetListClients(ListBox listbox)
        {
            ...
            while (reader.Read())
            {
                listbox.Items.Add($"{reader[0].ToString()}) ФИО: {reader[1].ToString()} | Пост: {reader[2].ToString()} | Номер: {reader[3].ToString()}");
            }
            reader.Close();
            conn.Close();
        }
        public bool InsertClients(string fio_employee, string phone_employee, string email_epmloyee)
        {
            int InsertCount = 0;
            bool result = false;
            conn.Open();
            string query = $"INSERT INTO employees (fio_employee, post_employee, phone_employee) VALUES ('{fio_employee}', '{phone_employee}', '{email_epmloyee}')";
            try
            {
